...
alpha_ran = torch.linspace(-1, 0, 9)
alpha = alpha_ran[task_id // 10]
...
for ii in range(len(m_ran)):
    ...
    print(f"alpha: {task_id // 10}, weight_decay: {task_id % 10}, m_idx: {ii}")

torch.save(results, f"../outputs/exp-10-3-{task_id}.pt")
torch.save(W_matrices, f"../outputs/exp-10-3-W-{task_id}.pt")
